...
process.on("uncaughtException", e => {
    signale.fatal(e);
    dialog.showErrorBox("Noge Progs crashed", e.message || "Cannot retrieve error message.");
    if (tty) {
        tty.close();
    }
    if (extraTtys) {
...
signale.start(`Starting Noge Progs v${app.getVersion()}`);
